---
figureType: table
spec:
  title: "Average Effective Tariff Rate by Trading Partner"
  subtitle: "Pre- and post-substitution"
  source: "GTAP v7, The Budget Lab analysis."
  scenario: "Section 122 Expires"
---

**Average Effective Tariff Rate by Trading Partner**

Pre- and post-substitution

_Scenario: Section 122 Expires_

_Notes: Three parallel metrics per country: Average Effective Tariff Rate (own rate, not additive), Share of Goods Imports, and Contribution (additive; sums to the Total). Eight trading partners: China, Canada, Mexico, EU, Japan, UK, FTA Partners, Rest of World. Table only, no chart, per standing preference._

_Source: GTAP v7, The Budget Lab analysis._
