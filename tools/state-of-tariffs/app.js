...
let manifest = null;
let renderModule = null;          // lazily-loaded { renderFigure, renderCurrentUpdate }
...
function renderSidebar() {
  ...
  if (tab.id === "current-update") {
    explainer.innerHTML =
      `<p>Use the tabs above to move between sections. Within each tab, the list below selects ` +
      `which figure to view; toggles and selectors (when present) switch between views of the data.</p>`;
...
async function renderMain() {
  ...
  try {
    if (tab.id === "current-update") {
      renderModule.renderCurrentUpdate(main, { body_html: tab.body_html });
      return;
    }
    const fig = figureById(tab, state.figure);
    if (!fig) {
